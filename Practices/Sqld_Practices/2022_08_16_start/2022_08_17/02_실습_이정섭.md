...
### 13. 음주(is_drinking)를 하는 사람(1)의 키(height)를 높은 순으로 5명 출력하시오. 

```sql
SELECT * FROM healthcare WHERE is_drinking = 1 AND height != '' ORDER BY height DESC LIMIT 5;
```

```
id      sido  gender  age  height  weight  waist  va_left  va_right  blood_pressure  smoking  is_drinking
------  ----  ------  ---  ------  ------  -----  -------  --------  --------------  -------  -----------
8832    30    1       10   195     95      99.0   0.9      0.7       168             2        1
46642   26    1       9    195     100     95.3   1.5      1.5       114             3        1
170951  28    1       12   195     85      76.0   1.5      1.5       117             1        1
219595  28    1       9    195     105     97.0   1.5      1.2       135             2        1
229284  28    1       9    195     105     100.0  1.0      1.0       131             1        1
```

### 14. 흡연(smoking)이 3이고 음주(is_drinking)를 하는 사람(1)은 몇 명인지 출력하시오.

```sql
SELECT COUNT(*) FROM healthcare WHERE is_drinking = 1 AND smoking = 3;
```
...
### 15. 흡연(smoking)이 3이고 음주(is_drinking)를 하는 사람(1)의 평균 키(height), 몸무게(weight), 허리 둘레(waist), 혈압(blood_pressure)을 구하시오.

```sql
SELECT AVG(height), AVG(weight), AVG(waist), AVG(blood_pressure) FROM healthcare WHERE smoking = 3 AND is_drinking = 1;
```

```
AVG(height)      AVG(weight)       AVG(waist)        AVG(blood_pressure)
---------------  ----------------  ----------------  -------------------
167.69321832124  69.4941839971801  85.4199599630194  125.151076409441
```